        ...
        while cap.isOpened():
            ...
            try:
                if results.pose_landmarks:
                    ...
                    # 计算左侧关节角度
                    l_shoulder_angle = calculate_angle(l_ear, l_shoulder, l_elbow)
                    ...
                    # 计算右侧关节角度
                    r_shoulder_angle = calculate_angle(r_ear, r_shoulder, r_elbow)
                    ...
                    # 计算平均角度
                    frame_data["joints"]["shoulder_angle"] = round(
                        (l_shoulder_angle + r_shoulder_angle) / 2, 2)
                    frame_data["joints"]["elbow_angle"] = round(
                        (l_elbow_angle + r_elbow_angle) / 2, 2)
                    frame_data["joints"]["hip_angle"] = round(
                        (l_hip_angle + r_hip_angle) / 2, 2)
                    frame_data["joints"]["knee_angle"] = round(
                        (l_knee_angle + r_knee_angle) / 2, 2)
                    frame_data["joints"]["ankle_angle"] = round(
                        (l_ankle_angle + r_ankle_angle) / 2, 2)
