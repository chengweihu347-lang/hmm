...
for label in unique_labels:
    ...
    if len(seqs_lab) == 0:
        continue
    # 取踝角并截断到 global_min_len
    ankle_mat = np.vstack([s[:global_min_len, 4] for s in seqs_lab])
    mean_curve = np.mean(ankle_mat, axis=0)
    std_curve = np.std(ankle_mat, axis=0)
...
plt.title("方法5：各类别平均踝角度（截断并插值后的结果）")
...
# ---------- 方法6：箱线图（每条序列的平均与标准差） ----------
ankle_mean = [np.mean(s[:, 4]) for s in sequences]
ankle_std = [np.std(s[:, 4]) for s in sequences]
df_stats = pd.DataFrame(
    {"mean_angle": ankle_mean, "std_angle": ankle_std, "label": labels}
)

plt.figure(figsize=(10, 5))
plt.subplot(1, 2, 1)
sns.boxplot(x="label", y="mean_angle", data=df_stats)
plt.title("方法6：每序列平均踝角度箱线图")
plt.subplot(1, 2, 2)
sns.boxplot(x="label", y="std_angle", data=df_stats)
plt.title("方法6：每序列踝角度标准差箱线图")
plt.tight_layout()
plt.show()
